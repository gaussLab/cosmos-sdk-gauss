...
func NewConfig() *Config {
	gip001Block, err := strconv.ParseInt(GIP001Block, 10, 64)
	if err != nil {
		gip001Block = -1
	}
...
func (config *Config) IsGIP001Block(height int64) bool{
	if config.gip001Block < 0 {
		return false
	}

	return (height > config.gip001Block)
}
