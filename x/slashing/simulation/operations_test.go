...
func TestSimulateMsgUnjail(t *testing.T) {
	...
	require.NoError(t, err)
	selfDelegation := stakingtypes.NewDelegation(val0AccAddress.Bytes(), validator0.GetOperator(), sdk.AccAddress(validator0.Address), issuedShares)
...
func getTestingValidator0(t *testing.T, app *simapp.SimApp, ctx sdk.Context, accounts []simtypes.Account) stakingtypes.Validator {
	...
	commissionReallocation0 := stakingtypes.NewCommissionReallocation(sdk.ZeroDec(), sdk.OneDec())
	return getTestingValidator(t, app, ctx, accounts, commission0, commissionReallocation0, 0)
}

func getTestingValidator(t *testing.T, app *simapp.SimApp, ctx sdk.Context, accounts []simtypes.Account, commission stakingtypes.Commission, 
	commissionReallocation CommissionReallocation, n int) stakingtypes.Validator {
	account := accounts[n]
