...
func (k Keeper) IncrementValidatorPeriod(ctx sdk.Context, val stakingtypes.ValidatorI) uint64 {
	...
	var currentRewards, currentCommission sdk.DecCoins
	if val.GetTokens().IsZero() {
		...
		feePool.CommunityPool = feePool.CommunityPool.Add(rewards.Rewards...).Add(rewards.ReallocatedCommission...)
		outstanding.Rewards = outstanding.GetRewards().Sub(rewards.Rewards).Sub(rewards.ReallocatedCommission)
		k.SetFeePool(ctx, feePool)
		k.SetValidatorOutstandingRewards(ctx, val.GetOperator(), outstanding)

		currentRewards = sdk.DecCoins{}
		currentCommission = sdk.DecCoins{}
	} else {
		// note: necessary to truncate so we don't allow withdrawing more rewards than owed
		currentRewards = rewards.Rewards.QuoDecTruncate(val.GetTokens().ToDec())
		currentCommission = rewards.ReallocatedCommission.QuoDecTruncate(val.GetTokens().ToDec())
	}

	// fetch historical rewards for last period
	historical := k.GetValidatorHistoricalRewards(ctx, val.GetOperator(), rewards.Period-1)
	rewardsHistorical := historical.CumulativeRewardRatio
	reallocatedCommissionHistorical := historical.CumulativeReallocatedCommissionRatio
	...
	k.SetValidatorHistoricalRewards(ctx, val.GetOperator(), rewards.Period, 
		types.NewValidatorHistoricalRewards(rewardsHistorical.Add(currentRewards...), reallocatedCommissionHistorical.Add(currentCommission...), 1))

	// set current rewards, incrementing period by 1
	k.SetValidatorCurrentRewards(ctx, val.GetOperator(), types.NewValidatorCurrentRewards(sdk.DecCoins{}, sdk.DecCoins{}, rewards.Period+1))
