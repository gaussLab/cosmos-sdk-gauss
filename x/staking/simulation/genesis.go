...
func RandomizedGenState(simState *module.SimulationState) {
	...
	for i := 0; i < int(simState.NumBonded); i++ {
		...
		commissionReallocation := types.NewCommissionReallocation(
			simulation.RandomDecAmount(simState.Rand, sdk.OneDec()),
			simulation.RandomDecAmount(simState.Rand, sdk.OneDec()),
		)
		...
		delegation := types.NewDelegation(simState.Accounts[i].Address, valAddr, sdk.AccAddress(valAddr), sdk.NewDec(simState.InitialStake))
