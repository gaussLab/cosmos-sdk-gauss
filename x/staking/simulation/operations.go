...
func SimulateMsgCreateValidator(ak types.AccountKeeper, bk types.BankKeeper, k keeper.Keeper) simtypes.Operation {
	return func(
		...
		commissionReallocation := types.NewCommissionReallocation(
			simtypes.RandomDecAmount(r, sdk.OneDec()),
			simtypes.RandomDecAmount(r, sdk.OneDec()),
		)

		msg, err := types.NewMsgCreateValidator(address, simAccount.ConsKey.PubKey(), selfDelegation, description, commission, commissionReallocation, sdk.OneInt())
		if err != nil {
...
func SimulateMsgEditValidator(ak types.AccountKeeper, bk types.BankKeeper, k keeper.Keeper) simtypes.Operation {
	return func(
		...
		newCommissionReallocationRate := simtypes.RandomDecAmount(r, sdk.OneDec())
		if err := val.CommissionReallocation.ValidateNewRate(newCommissionReallocationRate, ctx.BlockHeader().Time); err != nil {
			// skip as the commission is invalid
			return simtypes.NoOpMsg(types.ModuleName, types.TypeMsgEditValidator, "invalid commission reallocation rate"), nil, nil
		}
		...
		msg := types.NewMsgEditValidator(address, description, &newCommissionRate, &newCommissionReallocationRate, nil)
...
func SimulateMsgDelegate(ak types.AccountKeeper, bk types.BankKeeper, k keeper.Keeper) simtypes.Operation {
	return func(
		...
		recommanderAddr, _ := simtypes.RandomAcc(r, accs)
		...
		msg := types.NewMsgDelegate(simAccount.Address, val.GetOperator(), recommanderAddr.Address, bondAmt)
