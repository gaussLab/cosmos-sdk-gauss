...
var (
	...
	defaultCommissionReserveRate     = "1.0"
	defaultCommissionReallocatedRate = "0.6"
...
func NewCreateValidatorCmd() *cobra.Command {
	...
	cmd.Flags().AddFlagSet(FlagSetCommissionReallocation())
...
func NewEditValidatorCmd() *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			var newRate, newReserveRate *sdk.Dec
			...
			// get the initial validator commission reallaction parameters
			commissionReserveRate, _ := cmd.Flags().GetString(FlagCommissionReserveRate)
			if commissionReserveRate != "" {
				rate, err := sdk.NewDecFromStr(commissionReserveRate)
				if err != nil {
					return fmt.Errorf("invalid new commission reserve rate: %v", err)
				}
			...
			msg := types.NewMsgEditValidator(sdk.ValAddress(valAddr), description, newRate, newReserveRate, newMinSelfDelegation)
	...
	cmd.Flags().AddFlagSet(flagSetCommissionReallocationUpdate())
	cmd.Flags().AddFlagSet(FlagSetMinSelfDelegation())
	flags.AddTxFlagsToCmd(cmd)
...
`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			recommanderAddrStr, _ := cmd.Flags().GetString(FlagRecommander)
			if recommanderAddrStr != "" {
				recommanderAddr, err = sdk.AccAddressFromBech32(recommanderAddrStr)
				if err != nil {
					return err
				}
			}

			msg := types.NewMsgDelegate(delAddr, valAddr, recommanderAddr, amount)
			if err := msg.ValidateBasic(); err != nil {
				return err
			}
	...
	cmd.Flags().AddFlagSet(FlagSetRecommander())
...
func NewBuildCreateValidatorMsg(clientCtx client.Context, txf tx.Factory, fs *flag.FlagSet) (tx.Factory, sdk.Msg, error) {
	...
	// get the initial validator commission reallaction parameters
	reserveRateStr, _ := fs.GetString(FlagCommissionReserveRate)
	reallocatedRateStr, _ := fs.GetString(FlagCommissionReallocatedRate)

	commissionReallocation, err := buildCommissionReallocation(reserveRateStr, reallocatedRateStr)
	if err != nil {
		return txf, nil, err
	}
	...
	msg, err := types.NewMsgCreateValidator(
		sdk.ValAddress(valAddr), pk, amount, description, commissionRates, commissionReallocation,  minSelfDelegation,
	)
	if err != nil {
		return txf, nil, err
	}
...
func CreateValidatorMsgFlagSet(ipDefault string) (fs *flag.FlagSet, defaultsDesc string) {
	...
	fsCreateValidator.AddFlagSet(FlagSetCommissionReallocation())
	...
	commission reserve rate:     %s
	commission reallocate rate:  %s
	minimum self delegation:     %s
`, defaultAmount, defaultCommissionRate,
		defaultCommissionMaxRate, defaultCommissionMaxChangeRate,
		defaultCommissionReserveRate, defaultCommissionReallocatedRate,
...
type TxCreateValidatorConfig struct {
	...
	CommissionReserveRate   string
	CommissionReallocatedRate string
...
func PrepareConfigForTxCreateValidator(flagSet *flag.FlagSet, moniker, nodeID, chainID string, valPubKey cryptotypes.PubKey) (TxCreateValidatorConfig, error) {
	...
	c.CommissionReserveRate, err = flagSet.GetString(FlagCommissionReserveRate)
	if err != nil {
		return c, err
	}

	c.CommissionReallocatedRate, err = flagSet.GetString(FlagCommissionReallocatedRate)
	if err != nil {
		return c, err
	}
	...
	if c.CommissionReserveRate == "" {
		c.CommissionReserveRate = defaultCommissionReserveRate
	}

	if c.CommissionReallocatedRate == "" {
		c.CommissionReallocatedRate = defaultCommissionReallocatedRate
	}
...
func BuildCreateValidatorMsg(clientCtx client.Context, config TxCreateValidatorConfig, txBldr tx.Factory, generateOnly bool) (tx.Factory, sdk.Msg, error) {
	...
	reserveRateStr := config.CommissionReserveRate
	reallocatedRateStr := config.CommissionReallocatedRate
	commissionReallocation, err := buildCommissionReallocation(reserveRateStr, reallocatedRateStr)
	if err != nil {
		return txBldr, nil, err
	}
