...
type Helper struct {
	...
	CommissionReallocation stakingtypes.CommissionReallocation
	// Coin Denomination
	Denom string
}
...
func NewHelper(t *testing.T, ctx sdk.Context, k keeper.Keeper) *Helper {
	return &Helper{t, staking.NewHandler(k), k, ctx, ZeroCommission(), ZeroCommissionReallocation(), sdk.DefaultBondDenom}
}
...
func (sh *Helper) CreateValidatorMsg(addr sdk.ValAddress, pk cryptotypes.PubKey, stakeAmount sdk.Int) *stakingtypes.MsgCreateValidator {
	coin := sdk.NewCoin(sh.Denom, stakeAmount)
	msg, err := stakingtypes.NewMsgCreateValidator(addr, pk, coin, stakingtypes.Description{}, sh.Commission, sh.CommissionReallocation, sdk.OneInt())
...
func (sh *Helper) Delegate(delegator sdk.AccAddress, val sdk.ValAddress, amount sdk.Int) {
	coin := sdk.NewCoin(sh.Denom, amount)
	msg := stakingtypes.NewMsgDelegate(delegator, val, sdk.AccAddress(val), coin)
	sh.Handle(msg, true)
}
...
// ZeroCommissionReallocation constructs a commission-reallocation with all zeros.
func ZeroCommissionReallocation() stakingtypes.CommissionReallocation {
	return stakingtypes.NewCommissionReallocation(sdk.ZeroDec(), sdk.ZeroDec())
}
