...
func TestValidatorSetInitialCommissionReallocation(t *testing.T) {
	val := newValidator(t, valAddr1, pk1)
	testCases := []struct {
		validator   types.Validator
		commissionReallocation  types.CommissionReallocation
		expectedErr bool
	}{
		{val, types.NewCommissionReallocation(sdk.ZeroDec(), sdk.ZeroDec()), true},
		{val, types.NewCommissionReallocation(sdk.ZeroDec(), sdk.NewDecWithPrec(-1, 1)), true},
		{val, types.NewCommissionReallocation(sdk.ZeroDec(), sdk.NewDec(15000000000)), true},
		{val, types.NewCommissionReallocation(sdk.NewDecWithPrec(-1, 1), sdk.ZeroDec()), true},
		{val, types.NewCommissionReallocation(sdk.NewDecWithPrec(2, 1), sdk.NewDecWithPrec(1, 1)), true},
		{val, types.NewCommissionReallocation(sdk.ZeroDec(), sdk.NewDecWithPrec(1, 1)), true},
	}

	for i, tc := range testCases {
		val, err := tc.validator.SetInitialCommissionReallocation(tc.commissionReallocation)

		if tc.expectedErr {
			require.Error(t, err,
				"expected error for test case #%d with commission-reallocation: %s", i, tc.commissionReallocation,
			)
		} else {
			require.NoError(t, err,
				"unexpected error for test case #%d with commission-reallocation: %s", i, tc.commissionReallocation,
			)
			require.Equal(t, tc.commission, val.Commission,
				"invalid validator commission for test case #%d with commission-reallocation: %s", i, tc.commissionReallocation,
			)
		}
	}
}
