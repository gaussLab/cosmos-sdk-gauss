...
func TestMsgDecode(t *testing.T) {
	...
	commissionReallocation1 := types.NewCommissionReallocation(sdk.ZeroDec(), sdk.ZeroDec())
	msg, err := types.NewMsgCreateValidator(valAddr1, pk1, coinPos, types.Description{}, commission1, commissionReallocation1, sdk.OneInt())
	require.NoError(t, err)
...
func TestMsgDelegate(t *testing.T) {
	...
	for _, tc := range tests {
		msg := types.NewMsgDelegate(tc.delegatorAddr, tc.validatorAddr, sdk.AccAddress(tc.validatorAddr), tc.bond)
		if tc.expectPass {
			require.Nil(t, msg.ValidateBasic(), "test: %v", tc.name)
		} else {
			require.NotNil(t, msg.ValidateBasic(), "test: %v", tc.name)
