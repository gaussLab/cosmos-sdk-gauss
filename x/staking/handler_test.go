...
func TestInvalidCoinDenom(t *testing.T) {
	...
	recommanderAddr := sdk.AccAddress(valA)
	tstaking := teststaking.NewHelper(t, ctx, app.StakingKeeper)
	...
	commissionReallocation := types.NewCommissionReallocation(sdk.ZeroDec(), sdk.ZeroDec())
	msgCreate, err := types.NewMsgCreateValidator(valA, PKs[0], invalidCoin, types.Description{}, commission, commissionReallocation, sdk.OneInt())
	require.NoError(t, err)
	tstaking.Handle(msgCreate, false)

	msgCreate, err = types.NewMsgCreateValidator(valA, PKs[0], validCoin, types.Description{}, commission, commissionReallocation, sdk.OneInt())
	require.NoError(t, err)
	tstaking.Handle(msgCreate, true)

	msgCreate, err = types.NewMsgCreateValidator(valB, PKs[1], validCoin, types.Description{}, commission, commissionReallocation, sdk.OneInt())
	require.NoError(t, err)
	tstaking.Handle(msgCreate, true)

	msgDelegate := types.NewMsgDelegate(delAddr, valA, recommanderAddr, invalidCoin)
	tstaking.Handle(msgDelegate, false)

	msgDelegate = types.NewMsgDelegate(delAddr, valA, recommanderAddr, validCoin)
