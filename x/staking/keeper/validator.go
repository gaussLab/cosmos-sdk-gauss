...
// UpdateValidatorCommissionReallocation attempts to update a validator's commission reallocation rate.
// An error is returned if the new commission rate is invalid.
func (k Keeper) UpdateValidatorCommissionReallocation(ctx sdk.Context,
	validator types.Validator, newRate sdk.Dec) (types.CommissionReallocation, error) {
	commissionReallocation := validator.CommissionReallocation
	blockTime := ctx.BlockHeader().Time

	if err := commissionReallocation.ValidateNewRate(newRate, blockTime); err != nil {
		return commissionReallocation, err
	}

	commissionReallocation.ReserveRate = newRate
	commissionReallocation.UpdateTime = blockTime
