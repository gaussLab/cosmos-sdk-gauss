...
func TestUpdateValidatorCommissionReallocation(t *testing.T) {
	app, ctx, _, addrVals := bootstrapValidatorTest(t, 1000, 20)
	ctx = ctx.WithBlockHeader(tmproto.Header{Time: time.Now().UTC()})

	commission1 := types.NewCommissionReallocationWithTime(
		sdk.NewDecWithPrec(1, 1), sdk.NewDecWithPrec(3, 1),
		time.Now().UTC().Add(time.Duration(-1)*time.Hour),
	)
	commission2 := types.NewCommissionReallocation(sdk.NewDecWithPrec(1, 1), sdk.NewDecWithPrec(3, 1))

	val1 := teststaking.NewValidator(t, addrVals[0], PKs[0])
	val2 := teststaking.NewValidator(t, addrVals[1], PKs[1])

	val1, _ = val1.SetInitialCommissionReallocation(commission1)
	val2, _ = val2.SetInitialCommissionReallocation(commission2)
	...
	for i, tc := range testCases {
		commission, err := app.StakingKeeper.UpdateValidatorCommissionReallocation(ctx, tc.validator, tc.newRate)

		if tc.expectedErr {
			require.Error(t, err, "expected error for test case #%d with rate: %s", i, tc.newRate)
		} else {
			tc.validator.CommissionRallocation = commission
			app.StakingKeeper.SetValidator(ctx, tc.validator)
			val, found := app.StakingKeeper.GetValidator(ctx, tc.validator.GetOperator())
			...
			require.Equal(t, tc.newRate, val.CommissionReallocation.reserveRate,
				"expected new validator commission-reallocation rate for test case #%d with rate: %s", i, tc.newRate,
			)
			require.Equal(t, ctx.BlockHeader().Time, val.CommissionReallocation.UpdateTime,
				"expected new validator commission-reallocation update time for test case #%d with rate: %s", i, tc.newRate,
			)
		}
	}
}
